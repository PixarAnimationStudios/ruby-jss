...
class Hash
  ...
  # @example
  #   hash = {:foo => '', :bar => {:baz => '' }}
  #   hash.j_nillify!  # {:foo => nil, :bar => {:baz => '' }}
  #
  #   hash = {:foo => '', :bar => {:baz => '' }}
  #   hash.j_nillify! '', :recurse  # {:foo => nil, :bar => {:baz => nil }}
  #
  #   hash = {:foo => 123, :bar => {:baz => '', :bim => "123" }}
  #   hash.j_nillify! ['', 123], :recurse # {:foo => nil, :bar => {:baz => nil, :bim => "123" }}
  #
  #   hash = {:foo => 123, :bar => {:baz => '', :bim => "123" }}
  #   hash.j_nillify!(:anything, :recurse){|v| v.to_i == 123 }  # {:foo => nil, :bar => {:baz => '', :bim => nil }}
  #
  def j_nillify!(to_nils = '', recurse = false, &block)
    ...
    each_pair do |k, v|
      if v.class == Hash
        v.j_nillify!(to_nils, recurse, &block)
        next
      end
      do_it =
        if block_given?
  ...
  #
  #   computer_instance.hardware.j_ros.storage.first.partition.percentage_full
  #
  ...
  def j_recursive_ostruct
    @j_ros ||= RecursiveOpenStruct.new(self, recurse_over_arrays: true)
  end

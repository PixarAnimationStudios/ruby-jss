...
# TODO: This is probably not needed in Jamf module, since all time values should
# be Jamf::Timestamp instances, which is a subclass of Time.
...
class Time
  ...
  # def to_jss_epoch
  def j_to_epoch
    (self.strftime('%s') + self.j_msec.to_s.ljust(3,'0')).to_i
  end

  # TODO: Needed in JPAPI?
  # Dates/Timestamps should always be
  ...
  # def to_jss_utc
  def j_to_utc
    self.strftime("%Y-%m-%dT%H:%M:%S.#{self.j_msec}%z")
  end

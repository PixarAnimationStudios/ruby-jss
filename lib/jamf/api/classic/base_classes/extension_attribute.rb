...
module Jamf
  ...
  #   e.g. {Jamf::Computer}
  #
  # * ALL_TARGETS_CRITERION - a {Jamf::Criteriable::Criterion} instance that will be used in
  #   an {AdvancedSearch} to find all of members of the TARGET_CLASS
  #
  # @see Jamf::APIObject
  #
  class ExtensionAttribute < Jamf::APIObject
    ...
    # :id, :name, :in_jss, :need_to_update, and :rest_rsrc come from Jamf::APIObject
    ...
    def data_type=(new_val)
      ...
      raise Jamf::InvalidDataError, "data_type must be a string, one of: '#{DATA_TYPES.join("', '")}'" unless DATA_TYPES.include? new_val
    ...
    def input_type=(new_val)
      ...
      raise Jamf::InvalidDataError, "input_type must be a string, one of: #{INPUT_TYPES.join(', ')}" unless INPUT_TYPES.include? new_val
    ...
    def popup_choices=(new_val)
      ...
      raise Jamf::InvalidDataError, 'popup_choices must be an Array' unless new_val.is_a?(Array)
      ...
      new_val.map! do |v|
        ...
        case @data_type
        when DATA_TYPE_DATE
          raise Jamf::InvalidDataError, "data_type is Date, but '#{v}' is not formatted 'YYYY-MM-DD hh:mm:ss'" unless v =~ /^\d{4}(-\d\d){2} (\d\d:){2}\d\d$/
        when 'Integer'
          raise Jamf::InvalidDataError, "data_type is Integer, but '#{v}' is not one" unless v =~ /^\d+$/
        end
        v
      end
    ...
    def attribute_mapping=(new_mapping)
      new_mapping = Jamf::Validate.non_empty_string new_mapping.to_s
    ...
    def rest_xml
      ...
      unless self.class == Jamf::UserExtensionAttribute
